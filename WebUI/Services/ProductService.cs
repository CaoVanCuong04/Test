...
namespace WebUI.Services
{
    ...
    public class ProductService : IProductService
    {
        ...
        public async Task<List<Product>> GetAllProductsAsync(ProductLandingFilterType? filterType = null, int? categoryId = null, int currentPage = 1, int recordPerPage = 12)
        {
            try
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    options.Converters.Add(new NullableDecimalSafeConverter());
                    var apiResponse = JsonSerializer.Deserialize<ProductApiResponse>(json, options);
        ...
        // Custom converter for decimal? to handle invalid values gracefully
        public class NullableDecimalSafeConverter : System.Text.Json.Serialization.JsonConverter<decimal?>
        {
