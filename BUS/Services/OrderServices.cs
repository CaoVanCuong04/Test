...
namespace BUS.Services
{
    public class OrderServices : IOrderServices
    {
            ...
            try
            {
                ...
                await _unitOfWork.ExecuteInTransactionAsync(async () =>
                {
                    var order = new Order
                    {
                        ...
                        // Guest Order Fields - Lưu thông tin người nhận
                        Name = req.Name,
                        Phone = req.Phone,
                        Email = req.Email,
                        // GHN Address Fields
        ...
        public async Task<CommonPagination<GetListOrderRes>> GetListOrder(string? FullName, string? OrderCode, int? Status, DateTime? CreatedDate, int CurrentPage, int RecordPerPage)
        {
            ...
            // Use LEFT JOIN to support guest orders (UserID can be null)
            var query = from o in orders
                        join u in users on o.UserID equals u.UserID into userJoin
                        from u in userJoin.DefaultIfEmpty()
                        select new GetListOrderRes
                        {
                            OrderID = o.OrderID,
                            OrderCode = o.OrderCode,
                            // Ưu tiên lấy thông tin từ Order (khách vãng lai), nếu không có thì lấy từ User
                            FullName = o.Name ?? u.FullName ?? "Khách vãng lai",
        ...
        public async Task<CommonResponse<GetOrderDetailRes>> GetOrderDetail(int OrderID)
        {
            try
            {
                ...
                var orderRes = new GetOrderDetailRes
                {
                    ...
                    // Ưu tiên lấy thông tin từ Order (khách vãng lai), nếu không có thì lấy từ User
                    FullName = order.Name ?? order.User?.FullName ?? "Khách vãng lai",
